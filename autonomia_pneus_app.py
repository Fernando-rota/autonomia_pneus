...
def colorir_sulco(val):
    try:
        val_float = float(val)
        if val_float < 2: return "background-color: #FF6B6B; color: white"
        elif val_float < 4: return "background-color: #FFD93D; color: black"
        else: return "background-color: #6BCB77; color: white"
    except:
        return ""
...
def classificar_veiculo(desc):
    ...
    if "iveco" in d or "daily" in d or "dayli" in d or "scudo" in d: return "Utilitário (Iveco/Scudo)"
...
if arquivo:
    ...
    renames = {"Modelo": "Modelo (Atual)","SULCO": "Sulco","Sigla": "Sigla da Posição",
               "SIGLA": "Sigla da Posição","POSIÇÃO": "Posição"}
    ...
    map_model_novo = df_sulco[df_sulco["_VIDA"]=="NOVO"].dropna().drop_duplicates("_MODELO").set_index("_MODELO")["Sulco"].to_dict()
    ...
    df_pneus["% Sulco Restante"] = (df_pneus["Aferição - Sulco"] / df_pneus["Sulco Inicial"]*100).round(1)

    # ----------------- ABAS -----------------
    aba1, aba2, aba3, aba4 = st.tabs([
        "📌 Indicadores","📏 Medidas de Sulco","🗺️ Mapeamento dos Pneus","📝 Conclusão"
    ])
    ...
    with aba2:
        ...
        cols_show = [c for c in [
            ...
            "Desgaste (mm/km)","% Sulco Restante","Posição","Sigla da Posição"
        ...
        st.dataframe(
            df_show.style.applymap(colorir_sulco, subset=["Aferição - Sulco"])
                         .format({"Sulco Inicial":"{:.2f} mm","Aferição - Sulco":"{:.2f} mm",
                                  "Sulco Consumido":"{:.2f} mm","Desgaste (mm/km)":"{:.6f}",
                                  "% Sulco Restante":"{:.1f}%"}),
    ...
    # --- Mapeamento dos Pneus ---
    with aba3:
        st.subheader("🗺️ Mapeamento Visual dos Pneus")

        if "Posição" in df_pneus.columns and "Sigla da Posição" in df_pneus.columns:
            df_map = df_pneus.copy()
            df_map["SulcoStatus"] = pd.cut(
                df_map["Aferição - Sulco"],
                bins=[-1,2,4,99],
            ...
            fig_map = px.scatter(
                df_map,
                x="Posição", y="Sigla da Posição",
                color="SulcoStatus",
                size=np.where(df_map["Aferição - Sulco"].notna(), df_map["Aferição - Sulco"], 1),
                hover_data={
                    "Veículo - Placa": True,
                    "Modelo (Atual)": True,
                    "Vida": True,
                    "Aferição - Sulco": ":.2f",
                    "Km Rodado até Aferição": True,
                    "Posição": True
                },
                color_discrete_map={
                    "Crítico (<2mm)": "#FF6B6B",
            ...
            fig_map.update_traces(marker=dict(symbol="circle", line=dict(width=1, color="black")))
            fig_map.update_layout(
                xaxis_title="Posição do Veículo",
                yaxis_title="Eixo / Sigla da Posição",
            ...
            st.info("Mapa de posições não disponível — verifique se a aba 'posição' contém colunas 'Posição' e 'Sigla da Posição'.")
