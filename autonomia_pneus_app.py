...
def colorir_sulco(val):
    try:
        val_float = float(val)
        if val_float < 2: return "background-color: #FF6B6B; color: white; font-weight:bold"
        elif val_float < 4: return "background-color: #FFD93D; color: black; font-weight:bold"
        else: return "background-color: #6BCB77; color: white; font-weight:bold"
    except: return ""
...
if arquivo:
    ...
    if "Hodômetro Inicial" in df_pneus.columns: df_pneus["Hodômetro Inicial"] = df_pneus["Hodômetro Inicial"].apply(to_float)
    if "Vida do Pneu - Km. Rodado" in df_pneus.columns: df_pneus["Vida do Pneu - Km. Rodado"] = df_pneus["Vida do Pneu - Km. Rodado"].apply(to_float)
    else: df_pneus["Vida do Pneu - Km. Rodado"] = np.nan
    if "Observação" in df_pneus.columns: df_pneus["Observação - Km"] = df_pneus["Observação"].apply(extrair_km_observacao)
    else: df_pneus["Observação - Km"] = np.nan
    ...
    with aba2:
        ...
        # Barra de progresso
        st.markdown("### 🟢 % Sulco Restante")
        for _, row in df_show.iterrows():
            st.progress(min(int(row["% Sulco Restante"]),100))

        st.dataframe(
            df_show.style.applymap(colorir_sulco, subset=["Aferição - Sulco"])
                         .format({"Sulco Inicial":"{:.2f} mm","Aferição - Sulco":"{:.2f} mm",
                                  "Sulco Consumido":"{:.2f} mm","Desgaste (mm/km)":"{:.6f}","% Sulco Restante":"{:.1f}%"}),
