...
if arquivo:
    ...
    # ----------------- ABAS -----------------
    aba1, aba2, aba3, aba4 = st.tabs(["📌 Indicadores","📏 Medidas de Sulco","🚛 Mapeamento Truck","📝 Conclusão"])
    ...
    with aba3:
        ...
        for idx, row in df_pneus.iterrows():
            sigla = row.get("Sigla da Posição","")
            if sigla in posicoes_grid:
                grid_y, grid_x = posicoes_grid[sigla]
                x.append(grid_x)
                y.append(-grid_y)
                sulco = row.get("Aferição - Sulco", np.nan)
                if sulco >= 5:
                    color.append("green")
                elif sulco >= 3:
                    color.append("yellow")
                    ...
                    color.append("red")
                info = f"Placa: {row.get('Veículo - Placa','')}<br>Vida: {row.get('Vida','')} meses<br>Posição: {sigla}<br>Sulco: {sulco}mm"
                text.append(info)
                labels.append(sigla)
        fig_truck = go.Figure()
